...
def program():
    try:
        ...
        keys=wb['Acronym'].to_list()
        customer=str(frame2_dropdown3.get())
        values=wb[customer]
        message=""
        for index in range(len(keys)):
            data_message="{keys}={value}".format(keys=keys[index],value=values[index])
...
frame1_button2 = Button(master=frame1, text="Connect", command=connect,font=("Tahoma",10))
